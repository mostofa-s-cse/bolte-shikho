...
export function GoogleSignInButton() {
  ...
  async function handleClick() {
    ...
    const next = withLocale('/plan', locale)
  ...
  return (
    <button
      type="button"
      onClick={handleClick}
      className="flex w-full cursor-pointer items-center justify-center gap-2 rounded-full border border-border bg-surface px-5 py-2.5 text-sm font-semibold text-ink transition-colors hover:border-accent"
    >
      <GoogleIcon />
      {t.auth.googleButton}
    </button>
